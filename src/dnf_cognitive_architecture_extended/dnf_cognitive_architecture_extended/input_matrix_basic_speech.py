...
class InputMatrix(Node):
    ...
    def __init__(self):
        ...
        self.input_position = [-40, 0, 40]
        self.input_amplitude = 5.0
        self.input_width = 2.0
        self.input_duration = 1.0  # duration in seconds
        t_start_list = [1, 4, 7]
        t_stop_list = [2, 5, 8]

        if len(self.input_position) != len(t_start_list) or len(self.input_position) != len(t_stop_list):
            raise ValueError(
        ...
        self.gaussian_params_1 = [
            {'center': pos, 'amplitude': self.input_amplitude, 'width': self.input_width,
             't_start': t_start, 't_stop': t_stop}
            for pos, t_start, t_stop in zip(self.input_position, t_start_list, t_stop_list)
        ]
        ...
        self.input_matrix_2 = np.zeros((len(self.t), len(self.x)))
    ...
    def voice_command_callback(self, msg: String):
        if msg.data == "next_object":
            # get current simulation time
            current_time = self.t[self.current_time_index]
            duration = self.input_duration

            # Create Gaussian input that starts now and lasts `duration`
            gaussian_params = [
                {
                    'center': pos,
                    ...
                    't_stop': current_time + duration
                }
                for pos in self.input_position
            ]

            # Regenerate the matrix slice for input_matrix_2
            self.input_matrix_2 = self.get_input_matrix(
                gaussian_params, use_flat=True)

            self.get_logger().info(
                f"Triggered next_object at t={current_time:.2f}")
